...
async def get_khl_results(message: types.Message):
    buttons = [
        types.InlineKeyboardButton(text="Результаты за сегодняшний день", callback_data="today_results"),
        types.InlineKeyboardButton(text="Результаты за предыдущие дни", callback_data="previous_days_results"),
        types.InlineKeyboardButton(text="Запланированные матчи", callback_data="scheduled_matches")
    ]
...
@dp.callback_query_handler(text="today_results")
async def send_today_results(callback_query: types.CallbackQuery):
    url = "https://news.sportbox.ru/stats/2021-12-10"
    # url = f"https://news.sportbox.ru/stats/{datetime.date.today()}"
    headers = {
...
@dp.callback_query_handler(text="previous_days_results")
async def send_previous_results(call: types.CallbackQuery):
    await call.message.answer(str(randint(1, 10)))


@dp.callback_query_handler(text="scheduled_matches")
async def send_scheduled_matches(call: types.CallbackQuery):
    await call.message.answer(str(randint(1, 10)))
